...
import com.example.myweather.models.ClientErrorException
import com.example.myweather.models.InvalidApiKeyException
import com.example.myweather.models.NetworkException
import com.example.myweather.models.ServerErrorException
import com.example.myweather.models.WeatherNotFondException
...
class KtorWeatherApiImplementation(private val client: HttpClient) : WeatherApiService {
    ...
    override suspend fun getWeather(latitude: String, longitude: String): Result<WeatherResponse> {
        return try {
            val response = client.get(BASE_URL) {
                parameter(LATITUDE_QUERY, latitude)
                parameter(LONGITUDE_QUERY, longitude)
